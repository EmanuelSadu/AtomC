...
ALTERNATIVA ( a | b )
Se implementeaza testand pe rand fiecare componenta si returnand true la prima adevarata. Daca niciuna nu este
adevarata, in final se returneaza false.


Optionalitatea ( a? )
Deoarece in acest caz a poate sa existe sau nu, se incearca sa se consume acesta, dar fara sa se tina cont de
rezultatul incercarii.


Repetitia optionala ( a* )
Se include testarea lui a intr-o bucla care dureaza atata timp cat s-a putut consuma a.


. Prezenta ε ca membru intr-o
alternativa face ca acea alternativa sa devina optionala: „α | ε” = „α?”.
	 */
	...
	private boolean consumeExpRelResolved() {
		boolean er=false;
		if(currentAtomType() == Iduri.LESS || currentAtomType() == Iduri.GREATER || currentAtomType() == Iduri.LESSEQ || currentAtomType() == Iduri.GREATEREQ) {
			saveAtom();
			nextAtom();
			if(!consumeExprAdd()) {
				errorReporter.append(String.format(prefix,this.currentLine(), "Invalid Expr afrer ><== op\n"));
			...
			if(consumeExpr()){
				while(true){			
					if(currentAtomType()==Iduri.COMMA) {	
						nextAtom();
						if(! consumeExpr()) {
							errorReporter.append(String.format(prefix,this.currentLine(), "Missing Fct arg after COMMA )\n"));
							gotoNextDelimitator();
							return false;
						}
						continue;
